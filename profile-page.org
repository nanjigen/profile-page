...
This step sets up the Google Cloud CLI on the runner. It uses a pre-built action =google-github-actions/setup-gcloud@master= provided by Google. The =with= section provides configuration:
