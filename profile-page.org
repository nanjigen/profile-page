...
This step sets up the Google Cloud CLI on the runner. It uses a pre-built action [[https://github.com/google-github-actions/setup-gcloud][google-github-actions/setup-gcloud]] provided by Google. Note that we don't use the =main= branch, but instead =v1= as suggested by the action's warnings. The =with= section provides configuration:
